...
import { useGetTopChartsQuery, useLikeSongMutation, usePlaySongMutation, useGetFavouriteSongsQuery } from '../redux/services/CoreApi';
...
const TopCharts = () => {
  ...
  const { data: dataTopChart, isFetching: isLoadingTopChart, error: errorTopChart } =  useGetTopChartsQuery();
  ...
  const handleLike = async (songId, songName) => {
    ...
    try {
      const [{ request }] = await setLikeSongName(songId);
